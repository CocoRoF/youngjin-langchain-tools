...
# ============================================================
class _RunIdCaptureCallback:
    """
    Callback handler to capture the root run_id from LangGraph execution.

    This implements the LangChain callback interface to capture the actual
    run_id generated by LangGraph/LangChain for LangSmith feedback integration.

    Uses the BaseCallbackHandler interface from langchain_core.
    """

    def __init__(self) -> None:
        self.root_run_id: Optional[str] = None
        self._parent_run_ids: set = set()
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        """Capture run_id from LLM start if no root found yet."""
        if parent_run_id is None and self.root_run_id is None:
            self.root_run_id = str(run_id)
            logger.debug("Captured root run_id from LLM: %s", self.root_run_id)
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        """Capture run_id from chat model start if no root found yet."""
        if parent_run_id is None and self.root_run_id is None:
            self.root_run_id = str(run_id)
            logger.debug("Captured root run_id from ChatModel: %s", self.root_run_id)
